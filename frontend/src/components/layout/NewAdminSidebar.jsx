...
export default function NewAdminSidebar() {
  ...
  const allMenuItems = [
    ...
    { path: '/admin/tables', icon: <FaTable />, label: 'Quản lý bàn', roles: ['admin', 'manager'] },
    { path: '/admin/orders', icon: <FaShoppingCart />, label: 'Quản lý đơn hàng', roles: ['admin', 'manager', 'chef', 'waiter', 'staff'] },
    { path: '/admin/bookings', icon: <FaCalendarAlt />, label: 'Quản lý đặt bàn', roles: ['admin', 'manager', 'waiter', 'staff'] },
    { path: '/waiter/pos', icon: <FaCashRegister />, label: 'Màn hình POS', roles: ['admin', 'manager', 'waiter', 'staff'] },
  ...
  const isActive = (path) => {
    ...
    if ((user?.role === 'waiter' || user?.role === 'staff') && 
        (path === '/admin/orders' && location.pathname === '/orders' ||
         ...
         path === '/admin/notifications' && location.pathname === '/notifications' ||
  ...
  return (
    <div className="admin-sidebar bg-dark text-white" style={{ width: '250px', minHeight: '100vh' }}>
      <div className="logo p-3 border-bottom">
        <Link to={user?.role === 'waiter' || user?.role === 'staff' ? '/waiter/pos' : '/admin'} className="text-white text-decoration-none">
