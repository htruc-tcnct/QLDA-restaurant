  ...
  return (
    <div className="booking-management">
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)} size="lg">
        ...
        <Modal.Body>
          <Form onSubmit={handleSubmitEdit} id="booking-edit-form">
            <Row>
              <Col md={6}>
            ...
            {modalMode === 'view' && selectedBooking && (
              <>
                {selectedBooking.preOrderedItems && selectedBooking.preOrderedItems.length > 0 && (
                  <div className="mt-4">
                    ...
                    <Table striped bordered size="sm">
                      ...
                      <tbody>
                        {selectedBooking.preOrderedItems.map((item, index) => (
                          <tr key={index}>
                            <td>{item.menuItem?.name || 'N/A'}</td>
                            <td>{item.quantity}</td>
                            <td className="text-end">
                              {item.menuItem?.price
                                ? new Intl.NumberFormat('vi-VN').format(item.menuItem.price)
                                : 'N/A'}
                            </td>
                            <td className="text-end">
                              {item.menuItem?.price
                                ? new Intl.NumberFormat('vi-VN').format(item.menuItem.price * item.quantity)
                                : 'N/A'}
                            </td>
                          </tr>
                        ))}
                ...
                {selectedBooking.appliedPromotion && selectedBooking.appliedPromotion.code && (
                  <div className="mt-3">
                    <h5>Khuyến mãi áp dụng</h5>
                    <p>
                      <strong>Mã:</strong> {selectedBooking.appliedPromotion.code}
                      {selectedBooking.appliedPromotion.name && ` - ${selectedBooking.appliedPromotion.name}`}
                    </p>
                  </div>
                )}
                    ...
                    <Table borderless size="sm">
                      <tbody>
                        <tr>
                          ...
                          <td className="text-end">
                            {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(selectedBooking.paymentInfo.subtotal)}
                          </td>
                        </tr>
                        <tr>
                          <td><strong>Giảm giá:</strong></td>
                          <td className="text-end" style={{ color: 'red' }}>
                            - {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(selectedBooking.paymentInfo.discountAmount)}
                          </td>
                        </tr>
                        <tr>
                          <td><strong>Tổng tiền:</strong></td>
                          <td className="text-end fw-bold fs-5">
                            {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(selectedBooking.paymentInfo.totalAmount)}
                          </td>
                        </tr>
                      </tbody>
                    </Table>
        ...
        <Modal.Footer>
          ...
          {modalMode !== 'view' && (
            <Button variant="primary" type="submit" form="booking-edit-form">
              Lưu thay đổi
            </Button>
          )}
        </Modal.Footer>
