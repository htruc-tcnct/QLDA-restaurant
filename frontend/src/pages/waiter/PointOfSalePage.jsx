...
const PointOfSalePage = () => {
  ...
  const fetchMenuItems = async () => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data.menuItems)) {
        ...
        console.log(`Đã tải ${response.data.menuItems.length} món ăn thành công`);
      } else if (response.data && response.data.data && Array.isArray(response.data.data.menuItems)) {
        // Cấu trúc dự phòng nếu API thay đổi
        setMenuItems(response.data.data.menuItems);
        setFilteredMenuItems(response.data.data.menuItems);
        console.log(`Đã tải ${response.data.data.menuItems.length} món ăn thành công`);
      } else {
      ...
      if (error.response) {
        ...
        toast.error(`Không thể tải thực đơn: ${error.response.data.message || 'Lỗi máy chủ'}`);
  ...
  const fetchOpenOrders = async () => {
    try {
      const response = await api.get('/api/v1/orders?orderStatus=pending_confirmation,confirmed_by_customer,partially_served,fully_served,payment_pending');
  ...
  const handleMenuItemClick = (item, existingItem = null) => {
    ...
    if (existingItem) {
      ...
      const menuItem = menuItems.find(mi => mi._id === existingItem.menuItemId);
      if (menuItem) {
    ...
    try {
      ...
      if (currentOrder) {
        ...
        for (const item of existingItems) {
          await api.put(`/api/v1/orders/${currentOrder._id}/update-item/${item.id}`, {
            quantity: item.quantity,
            notes: item.notes
          });
        }
        ...
        if (status !== 'pending_confirmation' && status !== currentOrder.orderStatus) {
            ...
            <Card.Body className="p-0">
              ...
              {activeTab === 'orders' && (
                <div className="p-2">
                  ...
                  {openOrders.length === 0 ? (
                    <p className="text-center text-muted">Không có đơn hàng nào đang mở</p>
                  ) : (
                    <ListGroup>
                        ...
                        return (
                          <ListGroup.Item 
                            ...
                            onClick={() => {
                              ...
                              const table = tables.find(t => t._id === order.table?._id);
                              if (table) {
                                handleSelectTable(table);
                              } else {
                                toast.warning('Không tìm thấy thông tin bàn cho đơn hàng này');
                              }
                            }}
                            className="d-flex justify-content-between align-items-center"
                          >
                            <div>
                              <div><strong>{order.table?.name || 'Không có bàn'}</strong></div>
          ...
          <Card className="shadow-sm h-100">
            ...
            <Card.Body className="p-0 d-flex flex-column" style={{ height: 'calc(100% - 56px)' }}>
              <OrderDetails 
                ...
                disabled={savingOrder || (currentOrder && ['paid', 'cancelled_order'].includes(currentOrder.orderStatus))}
              />
              {renderOrderActions()}
            </Card.Body>
          </Card>
