...
const TableGrid = ({
  ...
  const checkUpcomingReservations = async () => {
    ...
    const tablesToCheck = tables.filter(
      (table) =>
    ...
    for (let i = 0; i < tablesToCheck.length; i += batchSize) {
      ...
      const promises = batch.map(async (table) => {
        try {
          const response = await tableService.getUpcomingReservations(
            table._id
          );
          ...
          console.error(
            `Error checking reservations for table ${table._id}:`,
            error
          );
          return false;
        }
        ...
        <div className="p-2">
          ...
          >
            {tables.map((table) => {
              ...
              const nextReservationInfo = table.nextReservationInfo;
              const hasUpcomingReservation =
                !!upcomingReservation || !!nextReservationInfo;
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    minWidth: "120px",
                    backgroundColor: getTableCardColor(
                      table.status,
                      isSelected,
                      isBookedAt,
                      hasUpcomingReservation
                    ),
                    border: hasUpcomingReservation
                      ? "2px solid #fd7e14"
                      : isSelected
                      ? "2px solid #007bff"
                      : "1px solid #dee2e6",
                ...
                >
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        backgroundColor: hasUpcomingReservation
                          ? "#fd7e14"
                          : isBookedAt
                          ? "#fd7e14"
                          : table.status === "available"
                      ...
                      <OverlayTrigger
                        ...
                        overlay={
                          <Tooltip>
                            ...
                            <div>
                              Thời gian: {formatBookingTime(bookingInfo)}
                            </div>
                      ...
                      <OverlayTrigger
                        ...
                        overlay={
                          <Tooltip>
                            ...
                            <div>
                              Thời gian:{" "}
                              {formatBookingTime(upcomingReservation)}
                            </div>
                            <div>
                              Số khách: {upcomingReservation.numberOfGuests}
                            </div>
                            <div>
                              Còn: {upcomingReservation.minutesUntil} phút nữa
                            </div>
                          </Tooltip>
                        }
                      >
                    ...
                    {/* Hiển thị thông tin đặt bàn từ nextReservationInfo */}
                    {!upcomingReservation && table.nextReservationInfo && (
                      <OverlayTrigger
                        placement="top"
                        overlay={
                          <Tooltip>
                            <div>
                              Khách: {table.nextReservationInfo.customerName}
                            </div>
                            <div>
                              SĐT: {table.nextReservationInfo.customerPhone}
                            </div>
                            <div>
                              Thời gian: {table.nextReservationInfo.date}{" "}
                              {table.nextReservationInfo.time}
                            </div>
                            <div>
                              Số khách:{" "}
                              {table.nextReservationInfo.numberOfGuests}
                            </div>
                            <div>
                              Còn: {table.nextReservationInfo.minutesUntil} phút
                              nữa
                            </div>
                          </Tooltip>
                        }
                      >
                        ...
                        >
                          <FaExclamationTriangle className="me-1" />
                          Còn{" "}
                          {table.nextReservationInfo.hoursUntil > 0
                            ? `${table.nextReservationInfo.hoursUntil}h${
                                table.nextReservationInfo.minutesUntil % 60
                              }p`
                            : `${table.nextReservationInfo.minutesUntil}p`}
                        </div>
                      </OverlayTrigger>
                    )}
