...
const isTableBookedAt = async (tableId, dateTime) => {
  ...
  console.log(`Checking if table ${tableId} is booked at ${bookingDate} ${bookingTime}`);
  ...
  if (bookings.length > 0) {
    for (const booking of bookings) {
      ...
      if (timeDifferenceInMinutes <= 15) {
        console.log(`Table ${tableId} is already booked within 15 minutes of the requested time`);
        return {
          isBooked: true,
          booking: booking,
...
exports.deleteTable = catchAsync(async (req, res, next) => {
  ...
  if (table.status === 'reserved') {
    return next(
      new AppError('Không thể xóa bàn đang được đặt trước', 400)
    );
  }
...
exports.clearTable = catchAsync(async (req, res, next) => {
  ...
  if (!['occupied', 'needs_cleaning'].includes(table.status)) {
    return next(new AppError('Chỉ có thể dọn bàn đang được sử dụng hoặc cần dọn dẹp', 400));
  }
...
exports.getAvailableTables = catchAsync(async (req, res, next) => {
  ...
  console.log(`Available tables: ${availableTables.length}, Unavailable due to conflicts: ${unavailableTables.length}`);
...
exports.markTableAsReserved = catchAsync(async (req, res, next) => {
  ...
  if (table.status === 'occupied') {
    return next(new AppError('Bàn đang được sử dụng, không thể đặt trước', 400));
  }
  ...
  if (bookingId) {
    ...
    const bookingDateTime = `${booking.date.toISOString().split('T')[0]}T${booking.time}:00`;
    ...
    if (bookingStatus.isBooked && bookingStatus.booking._id.toString() !== bookingId) {
  ...
  console.log(`Marking table ${table.name} (${table._id}) as reserved for booking ${bookingId}`);
